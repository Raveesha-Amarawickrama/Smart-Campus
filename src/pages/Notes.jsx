...
const TAG = {'Computer Science':'cs','Mathematics':'math','History':'hist','Physics':'sci','Philosophy':'sci','Economics':'econ','Biology':'bio','Engineering':'cs','English Literature':'eng','Psychology':'sci','Statistics':'math'};
...
export function NotesPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const legacy = notes.filter(n => n.imageData && !map[n.id]);

      if (legacy.length > 0) {
        await Promise.all(legacy.map(n => saveImage(n.id, n.imageData)));
        legacy.forEach(n => { map[n.id] = n.imageData; });
        const cleaned = notes.map(({ imageData, ...rest }) => rest);
        saveNotes(cleaned); 
      }
  ...
  const filtered = notes.filter(n=>{
    ...
    const mq = n.title.toLowerCase().includes(search.toLowerCase()) || n.subject.toLowerCase().includes(search.toLowerCase());
    return ms && mq;
  }).map(withImage);
  ...
  const addNote = async data => {
    ...
    const ok = await saveImage(id, imageData);
    if (!ok) toast('Note saved, but image storage failed (IndexedDB unavailable).', 'warning');
  ...
  return (
    <div className="page">
      <div className="pg">
        ...
        <div className="pg-sub">Photograph and store your handwritten lecture notes.</div>
        ...
        {!imgReady ? (
          <div className="empty">
            ...
            <div className="empty-p">Reading images from IndexedDB.</div>
          </div>
        ) : filtered.length===0 ? (
          <div className="empty">
            <div className="empty-icon"><Icon name="camera" size={28} color="var(--border2)"/></div>
...
function CameraView({onCapture, onClose, lastThumb}) {
  ...
  const shoot = ()=>{
    ...
    setCaptured(img); stopCam();
  };

  const retake = ()=>{ setCaptured(null); setReady(false); startCam(); };
    ...
    <div className="cam-shell">
      ...
      {!captured ? (
            ...
            <>
              ...
              <div className="cam-ov">
                <div className="cam-top">
                  ...
                  <span style={{color:'#fff',fontSize:16,fontWeight:800,letterSpacing:'-.2px'}}>Capture Notes</span>
                  <div style={{width:44}}/>
                </div>
                ...
                <div style={{flex:1,position:'relative',display:'flex',alignItems:'center',justifyContent:'center'}}>
                  <div className="cam-frame">
                    ...
                    {[['top:0,left:0','borderTop','borderLeft'],['top:0,right:0','borderTop','borderRight'],['bottom:0,left:0','borderBottom','borderLeft'],['bottom:0,right:0','borderBottom','borderRight']].map((_,i)=>{
                      ...
                      const borders   = [{borderTop:'3px solid var(--gold-light)',borderLeft:'3px solid var(--gold-light)'},{borderTop:'3px solid var(--gold-light)',borderRight:'3px solid var(--gold-light)'},{borderBottom:'3px solid var(--gold-light)',borderLeft:'3px solid var(--gold-light)'},{borderBottom:'3px solid var(--gold-light)',borderRight:'3px solid var(--gold-light)'}];
                      return <div key={i} style={{position:'absolute',width:20,height:20,...positions[i],...borders[i]}}/>;
                    })}
                  </div>
                  <div style={{position:'absolute',bottom:'calc(50% - 52% + 8px)',fontSize:12,color:'rgba(255,255,255,.8)',fontWeight:600}}>Align notes within frame</div>
                </div>
        ...
        <div style={{flex:1,display:'flex',flexDirection:'column',background:'#000'}}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'16px',background:'rgba(0,0,0,.6)'}}>
          ...
          <div style={{background:'var(--surface)',padding:20,display:'flex',flexDirection:'column',gap:14}}>
            <div className="fgrp">
              ...
              <input className="finput" placeholder="e.g. Advanced Calculus — Oct 24" value={form.title} onChange={e=>setForm(p=>({...p,title:e.target.value}))} autoFocus/>
            </div>
            <div className="fgrp">
            ...
            <button className="btn btn-navy" onClick={save} disabled={!form.title.trim()}>
