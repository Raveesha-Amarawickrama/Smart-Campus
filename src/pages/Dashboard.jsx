    ...
    <div className="page">
      <div className="pg">
        ...
        <div style={{ marginBottom: 14 }}>
          <div style={{ fontSize: 12, color: 'var(--t3)', fontWeight: 600 }}>{greeting},</div>
          <div style={{ fontSize: 24, fontWeight: 900, color: 'var(--navy)', letterSpacing: '-.5px', lineHeight: 1.2 }}>
            {user?.name?.split(' ')[0]} 👋
          </div>
          <div style={{ fontSize: 12, color: 'var(--t3)', marginTop: 4 }}>
            {pending.length > 0
              ? <><strong style={{ color: 'var(--gold)' }}>{pending.length} pending</strong>{urgent.length > 0 ? `, ${urgent.length} need attention` : ''}</>
        ...
        {urgent.length > 0 && (
          <div className="navy-card" style={{ padding: 18, marginBottom: 18 }}>
            <div style={{ position: 'relative', zIndex: 1 }}>
              ...
              <button
                onClick={() => complete(urgent[0].id)}
                style={{ background: 'rgba(255,255,255,.15)', border: '1.5px solid rgba(255,255,255,.3)', borderRadius: 10, padding: '9px 16px', color: '#fff', fontSize: 13, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer', minHeight: 48 }}
              >
        ...
        {scheduleLoading ? (
          ...
          <div style={{ marginBottom: 18 }}>
            {todaySched.map((item, i) => {
              ...
              const isDone = now.getHours() * 60 + now.getMinutes() > parse(item.end);
                ...
                <div key={item.id} className="sched-row">
                  ...
                  <div className={`sched-card ${isCurrent ? 'current' : ''}`} style={{ marginLeft: 10 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                      <span className={`sched-lbl ${isCurrent ? 'current' : ''}`}>
                        {isCurrent ? 'CURRENT SESSION' : isDone ? 'COMPLETED' : 'COMING UP'}
                      </span>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 18 }}>
          {[
            ...
            <div key={s.lbl} onClick={() => setPage('assignments')} style={{ background: s.bg, borderRadius: 14, padding: '13px 10px', cursor: 'pointer', border: `1.5px solid ${s.col}22`, textAlign: 'center', minHeight: 48 }}>
